...
export function PyramidSkyline({ size = 640, op = 0.6, color = "#E8C57A" }: { size?: number; op?: number; color?: string }) {
  const W = 640;
  const H = 260;
  const h = Math.round(size * (H / W));

  const PYR = [
    { ax: 220, ay: 26,  bx: 30,  cx: 400, by: 232 }, // Great
    { ax: 470, ay: 112, bx: 398, cx: 556, by: 232 }, // Mid
    { ax: 576, ay: 172, bx: 540, cx: 630, by: 232 }, // Small
  ];
  ...
  const courses = (p: { ax: number; ay: number; bx: number; cx: number; by: number }, n: number) => {
    ...
    return Array.from({ length: n }, (_, i) => {
      ...
      const hw = half * t;
      return <line key={i} x1={cx - hw} y1={y} x2={cx + hw} y2={y} stroke={color} strokeWidth={0.7} opacity={0.30 - i * 0.025} />;
    });
  };
  ...
  return (
    <svg width={size} height={h} viewBox={`0 0 ${W} ${H}`} fill="none" style={{ display: "block", pointerEvents: "none", opacity: op }}>
      ...
      {/* Soft sun behind the Great Pyramid */}
      ...
      {/* Sand dune base */}
      <path
        d="M0,238 C80,226 150,246 230,238 C320,228 400,244 470,236 C540,228 600,244 640,236 L640,260 L0,260 Z"
        fill="#C89A55"
        opacity={0.5}
      />
      <path
        d="M0,248 C110,240 200,252 320,246 C440,240 540,252 640,244 L640,260 L0,260 Z"
        fill="#8A5A2E"
        opacity={0.45}
      />
    </svg>
