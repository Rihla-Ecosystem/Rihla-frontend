...
const ALL_GOVERNORATES = ["Giza", "Cairo", "Luxor", "Aswan", "Alexandria", "Red Sea", "Sinai"];

const DEFAULT_BADGES = [
  { id: 1, name: "Giza Pioneer", icon: "🏔", color: C.sand },
  { id: 2, name: "Museum Maven", icon: "🏺", color: C.faience },
  { id: 3, name: "Cairo Chronicler", icon: "📜", color: C.copper },
  { id: 4, name: "Bazaar Navigator", icon: "🧭", color: "#A89880" },
  { id: 5, name: "Temple Scholar", icon: "🌿", color: "#A89880" },
  { id: 6, name: "Nile Wanderer", icon: "🌊", color: "#A89880" },
];
...
export default function HistoryPage() {
  ...
  // Formatting backend trip DTOs into UI view model
  ...
  const totalXP = userProfile?.xp ?? formattedVisits.reduce((sum, v) => sum + v.xp, 0);
  const currentLevel = userProfile?.level ?? 1;

  // Aggregate Governorates coverage
  const govVisits: Record<string, number> = {};
  ALL_GOVERNORATES.forEach((g) => {
    govVisits[g] = 0;
  });
        ...
        >
          <div>
            ...
            <p style={{ fontFamily: "'Inter',sans-serif", fontSize: "13px", color: `${C.limestone}55`, lineHeight: 1.6 }}>
              {userProfile?.displayName ? `${userProfile.displayName}'s journey log.` : "Every place you've been. Every story Rafiq told."}
            </p>
          </div>
          <div style={{ display: "flex", gap: 10 }}>
            {[
              ...
              >
                ...
                <div style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: "22px", fontWeight: 500, color: col }}>
                  {val}
                </div>
              </div>
            ))}
            ...
            <div style={{ position: "relative" }}>
              ...
              <div
                style={{
                  ...
                  bottom: 20,
                  width: 1.5,
                  background: `linear-gradient(180deg,${C.copper}40,${C.limestoneDark})`,
                  zIndex: 0,
                }}
              />
              <div style={{ display: "flex", flexDirection: "column", gap: 0 }}>
                        ...
                        <div style={{ display: "flex", justifyContent: "center", paddingTop: 16 }}>
                          <div
                            style={{
                              ...
                              height: 12,
                              borderRadius: "50%",
                              background: isOpen ? C.copper : C.limestoneDark,
                              border: `2px solid ${isOpen ? C.copper : "#C4B89A"}`,
                              boxShadow: isOpen ? `0 0 0 4px ${C.copper}20` : "none",
                              transition: "all 0.2s",
                              flexShrink: 0,
                            }}
                          />
                          ...
                          <div style={{ display: "grid", gridTemplateColumns: "90px 1fr auto", gap: 0 }}>
                            ...
                            <div style={{ padding: "12px 14px" }}>
                              <div style={{ display: "flex", alignItems: "center", gap: 7, marginBottom: 3 }}>
                                <span style={{ fontFamily: "'Inter',sans-serif", fontSize: "14px", fontWeight: 700, color: C.nile }}>
                                  {v.site}
                                </span>
                                {v.badge && (
                                  <span
                              ...
                              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                                ...
                                <span style={{ fontFamily: "'Inter',sans-serif", fontSize: "10px", color: "#A89880", background: C.limestoneDark, padding: "2px 7px", borderRadius: 99 }}>
                                  {v.cat}
                                </span>
                              </div>
                            </div>
                            <div style={{ padding: "14px 14px 0 0", display: "flex", alignItems: "flex-start" }}>
                              <ChevronRight
                                ...
                                color="#C4B89A"
                                strokeWidth={2}
                                style={{ transform: isOpen ? "rotate(90deg)" : "none", transition: "transform 0.2s", marginTop: 2 }}
                              />
                            </div>
                          </div>
                          {isOpen && (
                            <div style={{ padding: "0 16px 16px", borderTop: `1px solid ${C.copper}12`, marginTop: 2 }}>
                              ...
                              <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                                {v.tags.map((t) => (
                                  <span key={t} style={{ fontFamily: "'Inter',sans-serif", fontSize: "11px", fontWeight: 500, background: C.limestoneDark, color: "#8B7E6A", padding: "3px 9px", borderRadius: 99 }}>
                                    {t}
                                  </span>
                                ))}
                              </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16, position: "sticky", top: 24, alignSelf: "start" }}>
          ...
          <div style={{ background: `linear-gradient(135deg,${C.nile},${C.nileMid})`, borderRadius: 16, padding: "20px" }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 5 }}>
              <div style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: "20px", fontWeight: 500, color: C.limestone }}>
          ...
          <div style={{ background: C.limestone, borderRadius: 16, padding: "18px", border: "1px solid rgba(27,26,23,0.07)" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8 }}>
              {DEFAULT_BADGES.map((b) => {
                const earned = userBadges.some((ub) => ub.id === b.id || ub.name.toLowerCase() === b.name.toLowerCase());
                return (
                  <div
                    key={b.id}
                    style={{
                      ...
                      padding: "10px 6px",
                      borderRadius: 11,
                      background: earned ? `${b.color}10` : "#FAF7F0",
                      border: `1.5px solid ${earned ? b.color : "rgba(27,26,23,0.07)"}30`,
                      opacity: earned ? 1 : 0.45,
                    }}
                  >
                    <div style={{ fontSize: "22px", marginBottom: 5 }}>{b.icon}</div>
                    <div style={{ fontFamily: "'Inter',sans-serif", fontSize: "10px", fontWeight: earned ? 700 : 400, color: earned ? C.nile : "#A89880", lineHeight: 1.3 }}>
                      {b.name}
                    </div>
                  </div>
                );
          ...
          <div style={{ background: C.limestone, borderRadius: 16, padding: "18px", border: "1px solid rgba(27,26,23,0.07)" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              {Object.entries(govVisits).map(([name, count]) => (
                <div key={name} style={{ display: "grid", gridTemplateColumns: "80px 1fr 28px", gap: 10, alignItems: "center" }}>
                  <span style={{ fontFamily: "'Inter',sans-serif", fontSize: "12px", color: count > 0 ? C.nile : "#C4B89A", fontWeight: count > 0 ? 600 : 400 }}>
                    {name}
                  </span>
                  <div style={{ height: 6, background: "#EDE6D6", borderRadius: 99, overflow: "hidden" }}>
                    <div
                      style={{
                        height: "100%",
                        width: count > 0 ? `${Math.min(count * 25, 100)}%` : "0%",
                        background: count > 0 ? `linear-gradient(90deg,${C.copper},${C.sand})` : "transparent",
                        borderRadius: 99,
                        transition: "width 0.4s ease",
                      }}
                    />
                  </div>
                  <span style={{ fontFamily: "'Inter',sans-serif", fontSize: "11px", fontWeight: 700, color: count > 0 ? C.copper : "#C4B89A", textAlign: "right" }}>
                    {count}
                  </span>
                </div>
              ))}
            ...
            <div style={{ marginTop: 14, padding: "10px 12px", background: "#FAF7F0", borderRadius: 10, border: "1px solid rgba(27,26,23,0.06)" }}>
              <div style={{ fontFamily: "'Inter',sans-serif", fontSize: "11px", color: "#8B7E6A", lineHeight: 1.55 }}>
                You've explored <strong style={{ color: C.nile }}>{exploredGovCount} of 27</strong> governorates.
              </div>
            </div>
          </div>

          {/* Interaction Summary / Share */}
